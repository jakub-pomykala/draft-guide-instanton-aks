...
[role=command]
```
podman login instanton.azurecr.io --username 00000000-0000-0000-0000-000000000000 --password-stdin <<< $TOKEN
```
...
Pushing the image to a registry allows the cluster to create pods by using your container image. The registry that you are using is called Azure Container Registry.

Since you have already set up the Container Registry in the steps above now you just have to use podman to tag your image and push it to the Container Registry.
