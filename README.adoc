...
Replace the [hotspot=sysImage hotspot=sysImage hotspot=invImage file=0]`[registry-name]` with your container registry name.
...
for the [hotspot=sysImage file=1]`system` and [hotspot=invImage file=1]`inventory` repositories. These images can be pulled
with the [hotspot=sysSecret hotspot=invSecret file=1]`secret` that you defined before.

The service that is used to expose your deployments has a type of [hotspot=sysLoadBalancer hotspot=invLoadBalancer file=1]`LoadBalancer`. 
...
The service that is used to expose your deployments is [hotspot=createKnative file=1]`KNative`. This means that you can access theese services from the [hotspot=exposed file=1] `exposed` KNative endpoint. 
...
Set [hotspot=enable file=0]`enable` parameter to `true` in the [hotspot=semeruCloudCompiler1 file=0]`semeruCloudCompiler` configuration to grant application access to the Semeru Cloud Compiler server instance .
