...
:system-knative: [system-URL]/inventory/systems
:inventory-knative: [inventory-URL]/system/properties
:kube: Kubernetes
...
https://openliberty.io/docs/latest/instanton.html[Open Liberty InstantOn^] provides fast startup times for MicroProfile and Jakarta EE applications. Your applications can start in milliseconds, without compromising on throughput, memory, development-production parity, or Java language features. By using Semeru Cloud Compiler you'll be able to do just-in-time code compiling as a process independent of the applications virtual machine. Its benefits include CPU and memory utilization optimization in the application VM and improved resiliency.
...
image::checkpoint_diagram.png[checkpoint_diagram,align="center"]
...
image::JIT_diagram.png[JIT_Diagram,align="center"]
...
You will learn how to containerize a microservice with Open Liberty InstantOn and deploy the container with Semeru Cloud Compiler enablement to a Kubernetes cluster on Azure Kubernetes Servivces (AKS).
...
image::build_process_diagram.png[build_process_diagram,align="center"]
...
Starting from the [hotspot=kernel-slim file=1]`kernel-slim-java17-openj9-ubi` Liberty container image, build the container image by copying the local [hotspot=config file=1]`config` directory to the container `config` directory, running the [hotspot=features file=1]`features.sh` script to add the Liberty features that your microservice is required to operate, copying your microservice [hotspot=war file=1]`war` file, and running the [hotspot=configure file=1]`configure.sh` script to get a pre-warmed container image.

Finally, run the [hotspot=checkpoint file=1]`checkpoint.sh` script to make a checkpoint. The execution of the [hotspot=checkpoint file=1]`checkpoint.sh` must be the last instruction during your container image build. This configuration performs the application process checkpoint and stores the process data as the last layer of the application container image. In this guide, use the `afterAppStart` checkpoint option. For more information about the checkpoint options, see the https://openliberty.io/docs/latest/instanton.html#beforeAppStart[Faster startup with InstantOn^] documentation.
...
The three `--cap-add` options grant the three Linux capabilities that https://criu.org/Main_Page[CRIU](Checkpoint Restore In Userspace) requires to perform the application process checkpoint during the container image build. The `--security-opt` option grants access to all Linux system calls to the container image build.
...
By running the logs command you will be able to see the `system` service logs
[role=command]
```
kubectl logs system-6ff57df9-4zp2p
```

The below is part of the logs that you should expect to see
[role="no_copy"]
----
{"type":"liberty_message","host":"localhost","ibm_userDir":"\/opt\/ol\/wlp\/usr\/","ibm_serverName":"defaultServer","message":"CWWKF0011I: The defaultServer server is ready to run a smarter planet. The defaultServer server started in 0.148 seconds.","ibm_threadId":"0000003a","ibm_datetime":"2023-10-03T15:33:20.050+0000","ibm_messageId":"CWWKF0011I","module":"com.ibm.ws.kernel.feature.internal.FeatureManager","loglevel":"AUDIT","ibm_sequence":"1696347200050_000000000001F","ext_thread":"Default Executor-thread-1"}
----

If you inspect the above you can see "The defaultServer server started in 0.148 seconds." showing you that the InstantOn has successfully run
...
To configure the Semeru Cloud Compiler to handle Just-In-time (JIT) compilation requests from the application you have to set it's [hotspot=enable file=0]`enable` parameter to `true` and also specify the amount of replices you would like. Setting the [hotspot=enableSys hotspot=enableInv file=0]`enable` parameter to `true` the [hotspot=semeruCloudCompilerSys hotspot=semeruCloudCompilerInv file=0]`semeruCloudCompiler` configuration is granted the applications access to the Semeru Cloud Compiler server instance.

[role="code_command hotspot file=0", subs="quotes"]
----
#Replace the `kubernetes.yaml`.#
`kubernetes.yaml`
----

kubernetes.yaml
[source, yaml, linenums, indent=0, role="code_column hide_tags=createKNativeSys,createKNativeInv,env"]
----
include::finish/kubernetes.yaml[]
----
...
By using KNative serving you have the benefit of rapid deployment of serverless containers. 

To have KNative serving available it requires a few installtion steps to be run.

Firstly you might be required to give permissions the `configure-knative.sh` script.
You can do this by running:
[role=command]
```
chmod +x scripts/configure-knative.sh
```
...
Now you can apply the Knative Serving deployment by running 
[role=command]
```
kubectl apply -f kubernetes.yaml
```

You can verify that KNative was successfully installed by running the following command:
[role=command]
```
k get pods -A | grep knative
```

You should see output similair to the below:
[role="no_copy"]
----
knative-serving   activator-7499d967fc-6d4xp                1/1     Running     0          10m
knative-serving   autoscaler-568989dd8c-7hl9l               1/1     Running     0          10m
knative-serving   controller-76c798ffcb-xh7vs               1/1     Running     0          10m
knative-serving   default-domain-bpzxs                      0/1     Completed   0          10m
knative-serving   domain-mapping-7748ff49d4-wvcdn           1/1     Running     0          10m
knative-serving   domainmapping-webhook-755d864f5c-kgwc2    1/1     Running     0          10m
knative-serving   net-kourier-controller-79c998474f-db645   1/1     Running     0          10m
knative-serving   webhook-8466d59795-5kjsk                  1/1     Running     0          10m
----
...
To enable KNative serving add in the [hotspot=createKNativeSys hotspot=createKNativeInv file=0]`CreateKnativeService` to both the `system` and `inventory` service and setting it to `enable`.
...
Run the following command to re apply the deployment to the `system` and `inventory` InstantOn images to your AKS:
[role=command]
```
kubectl apply -f kubernetes.yaml
```

These services will now be KNative services instead of normal kubernetes services. To get the addresses for these run:
[role='command']
```
kubectl get ksvc
```

Upon running the above you will see the following:

[role="no_copy"]
----
NAME        URL                                              LATESTCREATED     LATESTREADY       READY   REASON
inventory   http://inventory.default.20.26.166.49.sslip.io   inventory-00001   inventory-00001   False   NotOwned
system      http://system.default.20.26.166.49.sslip.io      system-00001      system-00001      False   NotOwned
----

Once the services are ready you should expect to see something similair to below: 
[role="no_copy"]
----
NAME        URL                                              LATESTCREATED     LATESTREADY       READY   REASON
inventory   http://inventory.default.20.26.166.49.sslip.io   inventory-00001   inventory-00001   True
system      http://system.default.20.26.166.49.sslip.io      system-00001      system-00001      True
----

Because the have the `system` and  `inventory` still have [hotspot=exposeSystem hotspot=exposeInv file=0]`expose` parameter set to `true` you can now access these services using the URLs provided in the output above. 

You can now access the microservices using the KNative endpoints but you also have to pass an env variable to the inventory service to accept port 80 as system.http.port instead of port 9080. This is because KNative deploys the system service on port 80 and now 9080.

Point your browser to the `{inventory-knative}/[system-KNative-URL]` URL. When you visit this URL, these system
properties are automatically stored in the inventory. Go back to `{inventory-knative}` and
you see a new entry for `[system-KNative-URL]`.

// =================================================================================================
// Testing microservices that are running on Azure AKS
// =================================================================================================
