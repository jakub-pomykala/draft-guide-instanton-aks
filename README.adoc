...
kubernetes.yaml
[source, yaml, linenums, indent=0, hide_tags=createKNativeSys,createKNativeInv, role="code_column"]
----
...
The service that is used to expose your deployments is [hotspot=createKNativeSys hotspot=createKNativeInv file=0]`KNative`. This means that you can access theese services from the [hotspot=exposeSystem hotspot=exposeInv file=0]`exposed` KNative endpoint. 
...
The [hotspot file=0]`kubernetes.yaml` file is configured to deploy two of resources of the [hotspot=olappSys hotspot=olapp file=0]`OpenLibertyApplication` kind, [hotspot=system file=0]`system` and [hotspot=inventory file=0]`inventory`, which is controlled by the Open Liberty Operator.
...
Set [hotspot=enable file=0]`enable` parameter to `true` in the [hotspot=semeruCloudCompilerSys file=0]`semeruCloudCompiler` configuration to grant application access to the Semeru Cloud Compiler server instance .
