...
Replace the [hotspot=sysImage hotspot=sysImage hotspot=invImage file=2]`[registry-name]` with your container registry name.
...
The [hotspot=sysImage hotspot=invImage file=2]`image` is the name and tag of the container image that you want 
to use for the container. The [hotspot]`kubernetes.yaml` file references the images that you pushed to your registry
for the [hotspot=sysImage file=2]`system` and [hotspot=invImage file=2]`inventory` repositories. These images can be pulled
with the [hotspot=sysSecret hotspot=invSecret file=2]`secret` that you defined before.

The service that is used to expose your deployments has a type of [hotspot=sysLoadBalancer hotspot=invLoadBalancer file=2]`LoadBalancer`. 
...
The service that is used to expose your deployments is [hotspot=createKnative file=2]`KNative`. This means that you can access theese services from the [hotspot=exposed file=2] `exposed` KNative endpoint. 
...
[role="code_command hotspot file=2", subs="quotes"]
----
#Update the kubernetes.yaml configuration file in the start directory.#
`kubernetes.yaml`
----
...
The [hotspot file=2]`kubernetes.yaml` file is configured to deploy two of resources of the [hotspot=olappSys hotspot=olapp file=2]`OpenLibertyApplication` kind, [hotspot=system file=2]`system` and [hotspot=inventory file=2]`inventory`, which is controlled by the Open Liberty Operator.

The [hotspot=sysImage file=2]`applicationImage` parameter defines what container image is deployed as part of the `OpenLibertyApplication` CRD. Update [hotspot=sysImage file=2]`[system-repository-uri]` in the `applicationImage` field to point to your `system` repository URI.

In [hotspot=securityContext file=2]`securityContext` configuration, the [hotspot=runAsNonRoot file=2]`runAsNonRoot` cofiguration specifies that for any configuration in the pod, all processes run with a non-root user ID. With the [hotspot=capabilities file=2]`capabailities` configuration, we are only adding the the  `CHECKPOINT_RESTORE` and `SETCAP` capabilities for our InstantOn image and dropping all the remaining capabilities by specifying [hotspot=ALL file=2]`ALL` in the `capabilities.drop`. 

emeru Cloud Comiler server instance provides real-time code compiling as a process, independent of the application.

Set [hotspot=enable file=2]`enable` parameter to `true` in the [hotspot=semeruCloudCompilerSys file=2]`semeruCloudCompiler` configuration to grant application access to the Semeru Cloud Compiler server instance .

To configure the Semeru Cloud Compiler to handle Just-In-time (JIT) compilation requests from the application you have to set it's [hotspot=enable file=2]`enable` parameter to `true` and also specify the amount of replices you would like.
...
You can now make a request to `[hostname]:31000`, you might need to wait a few minutes before you try to access the `system` microservice.
...
You just deployed two microservices running in Open Liberty to Azure AKS. You also learned how to use the `kubectl` command to deploy your microservices on a kubernetes cluster.
