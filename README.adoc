...
The service that is used to expose your deployments is [hotspot=createKnative] `KNative`. This means that you can access theese services from the [hotspot=exposed] `exposed` KNative endpoint. 
...
The [hotspot file=2]`kubernetes.yaml` file is configured to deploy two of resources of the [hotspot=olappSystem hotspot=olapp file=0]`OpenLibertyApplication` kind, [hotspot=system file=0]`system` and [hotspot=inventory file=0]`inventory`, which is controlled by the Open Liberty Operator.
