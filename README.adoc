...
* *Azure CLI :* The Azure CLI is a cross-platform command line tool that allows you to connect to Azure and execute commands on Azure resources. See the offical https://learn.microsoft.com/en-us/cli/azure/install-azure-cli-linux?pivots=apt[Install the Azure CLI on Linux^] documentation for information about downloading andsetting up `Azure CLI` on your Linux platform.

* *Azure Azure Kubernenetes Services:* Azure Azure Kubernenetes Services (AZ AKS) is used to manage Azure Kubernetes Services. You can install this by running:
...
Before you configure the Azure CLI, you need to be logged in to get access to your subscription.
...
Your private container registry manages Podman images that you build in later steps. 
...
`start/system` and `start/inventory`. Both of these directories contain a Containerfile, which is necessary
for building the Podman images. If you're unfamiliar with Containerfile, check out the
...
Podman images using the `podman build` command. To build your image, you need to have Podman installed 
and your Podman daemon started.
...
Your two images `system` and `inventory` should appear in the list of all Podman images:
...
[role=command]
```
kubectl create secret podman-registry guidesecret \
    --podman-server=[registry-server] \
    --podman-username=[registry-name] \
    --podman-password=[password] \
    --podman-email=[email-address]
```
...
The service that is used to expose your deployments is [hotsport=createKnative] `KNative`. This means that you can access theese services from the [hotspot=exposed] `exposed` KNative endpoint. 
...
[role="no_copy"]
----
openlibertyapplication.apps.openliberty.io/system created
----
...
olo-controller-manager-b99549758-2ndw9      1/1     Running   0          65s
system-6c594b4fb-jvzsc                      1/1     Running   0          20s
system-semeru-compiler-1-64c696b88b-l5kqq   1/1     Running   0          41s
system-semeru-compiler-1-64c696b88b-rlgkt   1/1     Running   0          41s
system-semeru-compiler-1-64c696b88b-wnnnz   1/1     Running   0          41s
----
...
----
#Create the kubernetes.yaml configuration file in the start directory.#
`kubernetes.yaml`
----
...
The [hotspot file=0]`kubernetes.yaml` file is configured to deploy one [hotspot=olapp file=0]`OpenLibertyApplication` resource, [hotspot=system file=0]`system`, which is controlled by the Open Liberty Operator.
...
Set [hotspot=enable file=0]`enable` parameter to `true` in the [hotspot=semeruCloudCompiler file=0]`semeruCloudCompiler` configuration to grant application access to the Semeru Cloud Compiler server instance .
